  <?php
    ...
    // Requête get all Pokémons
      ...
      // Add Pokémon
      function addPokemon(PDO $dsn, $name, $pv, $type1, $type2, $editable) {
        // var_dump("type 2 : " . $type2);
        if (!$type2) {
          $sql = "INSERT INTO pk_list (Id, Name, Pv, Image, Type1, Type2, Editable) VALUES (NULL, :name, :pv, 'Default.png', :type1, NULL, :editable);";
          $stmt = $dsn->prepare($sql);
        } else {
          $sql = "INSERT INTO pk_list (Id, Name, Pv, Image, Type1, Type2, Editable) VALUES (NULL, :name, :pv, 'Default.png', :type1, :type2, :editable);";
          $stmt = $dsn->prepare($sql);
